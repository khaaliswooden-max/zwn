import { Driver } from 'neo4j-driver';
import { v4 as uuidv4 } from 'uuid';
import { RelianStatePayload } from '../parsers/relian-parser';

export async function writeMigrationState(
  driver: Driver,
  payload: RelianStatePayload,
  solanaSlot: number,
  txSignature: string
): Promise<string> {
  const session = driver.session();
  const stateId = uuidv4();
  const eventId = uuidv4();
  const now = Date.now();

  try {
    await session.executeWrite(async (tx) => {
      await tx.run(
        `MERGE (a:WorldActor {id: $projectId})
           ON CREATE SET a.created_at = $now
           SET a.last_seen = $now
         WITH a
         CREATE (s:MigrationState {
           id: $stateId,
           project_id: $projectId,
           semantic_preservation: $semanticPreservation,
           test_coverage: $testCoverage,
           velocity_loc_day: $locMigrated,
           artifact_hash: $artifactHash,
           timestamp: $timestamp,
           solana_slot: $solanaSlot,
           tx_signature: $txSignature,
           is_current: true
         })
         WITH a, s
         OPTIONAL MATCH (a)-[:HAS_STATE]->(prev:MigrationState {is_current: true})
           WHERE prev.id <> s.id
         SET prev.is_current = false
         WITH a, s, prev
         FOREACH (_ IN CASE WHEN prev IS NOT NULL THEN [1] ELSE [] END |
           CREATE (s)-[:SUPERSEDES {at: $now}]->(prev)
         )
         WITH a, s
         CREATE (a)-[:HAS_STATE {since: $timestamp, source: 'relian'}]->(s)
         WITH s
         CREATE (e:SubstrateEvent {
           id: $eventId,
           type: 'MIGRATION_COMPLETE',
           source: 'relian',
           entity_id: $projectId,
           payload_hash: $payloadHash,
           solana_slot: $solanaSlot,
           timestamp: $timestamp
         })
         CREATE (s)-[:EMITTED]->(e)`,
        {
          stateId, projectId: payload.projectId,
          semanticPreservation: payload.semanticPreservation,
          testCoverage: payload.testCoverage, locMigrated: payload.locMigrated,
          artifactHash: Buffer.from(payload.artifactHash).toString('hex'),
          timestamp: payload.timestamp, solanaSlot, txSignature,
          payloadHash: `migration:${payload.projectId}:sp=${payload.semanticPreservation}`,
          eventId, now,
        }
      );
    });

    console.log(`[migration-writer] Wrote MigrationState ${stateId} + SubstrateEvent ${eventId} for project ${payload.projectId}`);
    return eventId;
  } finally {
    await session.close();
  }
}
